...
function isArray(target) {
  if (Object.prototype.toString.call(target) === "[object Array]") {
    ...
    console.error(`${target} type is not Array`);
    process.exit(1);
  }
}
...
function getWriterOpts(options) {
  return {
    transform: (commit, context) => {
      ...
      let typeList;
      if (configOpts.hasOwnProperty("types")) {
        if (isArray(configOpts.types) && configOpts.types.length > 0) {
          typeList = configOpts.types;
        } else {
          typeList = defaultOpts.types;
        }
      } else {
        typeList = defaultOpts.types;
      }

      if (configOpts.hasOwnProperty("authorName") && isBoolean(configOpts.authorName) && configOpts.authorName) {
        commit.authorName = commit.authorName;
      } else {
        delete commit["authorName"];
      }

      if (configOpts.hasOwnProperty("authorEmail") && isBoolean(configOpts.authorEmail) && configOpts.authorEmail) {
        commit.authorEmail = commit.authorEmail;
      } else {
        delete commit["authorEmail"];
      }

      const target = typeList.filter((item) => item.type === commit.type).pop();
      if (target) {
...
module.exports = Q.all([
  ...
  try {
    ...
    if (hasConfigFile) {
      ...
      const configPath = resolve(pkgPath, ".", configName);
